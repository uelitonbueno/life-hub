...
import { calculateFinanceSummary, getDashboardData } from "../client/src/lib/lifehub-calculations";
import type { CalendarEvent, DocumentRecord, FinancialRecord } from "../client/src/lib/lifehub-domain";

const timestamp = (value: string) => new Date(value).toISOString();

describe("regras de negócio do Life Hub", () => {
  it("calcula totais financeiros por status e categoria a partir dos registros persistidos", () => {
    ...
    expect(summary).toMatchObject({ total: 1730, paid: 1200, pending: 350, overdue: 180 });
  ...
  it("monta o dashboard com documentos ativos, contas abertas e o próximo evento", () => {
    ...
    expect(dashboard).toMatchObject({ documentsToReview: 1, activeDocuments: 1, unpaidBills: 1 });
